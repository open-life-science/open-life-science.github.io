...
We welcome our {{ p-mentors | size }} mentors, {{ p-mentors | join: ', ' }}, based in {{ m-countries | size }} countries ({{ m-countries | join: ', ' }}). {{ prev-part-count }} of them were participants and {{ prev-mentor-count }} mentors in the [previous cohort (OLS-3)](/openseeds/ols-3). They will be supported by [{{ experts | size }} experts](/openseeds/{{ cohort }}#experts).
