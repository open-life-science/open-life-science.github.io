...
{% assign projects = site.data.openseeds.ols-1.projects %}
...
{% for project in projects %}
    {% if project.status == 'graduated' %}
        ...
        {% for p in project.participants %}
            {% capture grad-participants %}{{ grad-participants}}, [{{ people[p].first-name }} {{ people[p].last-name }}]({% link openseeds/ols-1/projects-participants.md %}#{{ p }}){% endcapture %}
        {% endfor %}
        {% for m in project.mentors %}
            {% capture grad-mentors %}{{ grad-mentors }}, [{{ people[m].first-name }} {{ people[m].last-name }}]({% link openseeds/ols-1/index.md %}#{{ m }}){% endcapture %}
        {% endfor %}
    {% endif %}
    {% for p in project.participants %}
        {% capture all-participants %}{{ all-participants}}, [{{ people[p].first-name }} {{ people[p].last-name }}]({% link openseeds/ols-1/projects-participants.md %}#{{ p }}){% endcapture %}
    {% endfor %}
    {% for m in project.mentors %}
        {% capture all-mentors %}{{ all-mentors }}, [{{ people[m].first-name }} {{ people[m].last-name }}]({% link openseeds/ols-1/index.md %}#{{ m }}){% endcapture %}
    {% endfor %}
{% endfor %}
...
{% assign schedule = site.data.openseeds.ols-2.schedule %}
{% include timeline.md %}
