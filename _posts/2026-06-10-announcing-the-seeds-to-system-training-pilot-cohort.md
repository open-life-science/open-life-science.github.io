---  
layout: post
title: 'Announcing the Seeds to Systems Training Pilot Cohort'
authors:
- iramosp
- rivaquiroga
image: https://images.unsplash.com/photo-1440342359743-84fcb8c21f21?fm=jpg&q=80&w=1400&auto=format&fit=crop
photos:
  name: kazuend
  license: Unsplash License
  url: https://unsplash.com/@kazuend
---

**We are thrilled to announce that the pilot cohort of Seeds to Systems will run September to November 2026**. 

## What is the Seeds to Systems Training Programme?

As part of our [organisational strategy]({% link _posts/2025-06-30-seeds-to-system-strategy.md %}), Seeds to Systems is a cohort-based training and mentoring programme designed for individuals and teams leading open initiatives, including research communities of practice,  volunteer-run research initiatives, and research adjacent projects, such as software and data projects. It builds directly on OLS's experience running [Open Seeds]({% link openseeds/index.md %}) and extends that support to projects at a more advanced stage of development.

The curriculum centres on **organisational infrastructure for project and community sustainability**: the practical knowledge, skills, and networks needed to lead a project over time, navigate the research funding landscape, and build the conditions that enable long-term impact.

Current (tentative!) curriculum outline

The pilot curriculum is still under development, and we expect it to include the following topics:

* Networks and collaboration  
* Outreach and visibility  
* Community governance as infrastructure (policies, documentation, risk management, governance models)  
* Impact (designing meaningful impact frameworks and shaping impact narratives)  
* Navigating the research funding landscape, [including grant witing]({% link _posts/2026-04-20-from-hidden-curriculum-to-grant-readiness.md %})
* Critical AI use as a cross-cutting theme across these topics

Participants will work through the curriculum as a cohort, learning from each other's experiences and from a network of mentors and experts. The programme will run from September to November/December 2026.

## How to contribute and engage

### Webinar

Learn more about the Seeds to Systems project and opportunities to contribute in our upcoming webinar. [Register here](https://us02web.zoom.us/meeting/register/ln9HJVbIR9-IAeWedXIG7g).

### Community consultation workshops

We are inviting members of the open research community to participate in two curriculum consultation workshops to shape the scope of the Seeds to Systems training programme:

- **Leverage points for project and community sustainability:** What tools and strategies are most impactful in securing funding across diverse organisational and individual contexts?  
- **Pathways towards a sustainable open research ecosystem:** What visions do we have for a more sustainable open research ecosystem, and how do we build pathways for those visions to become possible over the next five years?

Learn more and [register here]({% link _posts/2026-06-10-seeds-to-system-training-community-consultation-workshops.md %}).

### Experts and Knowledge Sharing

We are also inviting people with practical experience in leading and sustaining open research communities to contribute to the development and delivery of the curriculum. Contributions might include joining a community consultation workshop, reviewing curriculum materials, or participating as a trainer or mentor during the pilot cohort. Details will be shared soon. 

### Pilot cohort

The pilot cohort is designed for individuals or teams who already have experience running their own open science initiatives, and who are interested in planning for long term sustainability, including, but not limited to, securing funding. 

Participants from the OLS community and projects that have participated in Open Seeds are likely to be strong candidates for the pilot. We view the pilot cohort as an opportunity to shape the curriculum.

Time commitment: We are finalising the cohort structure. The expected time commitment for participants is 5h per week for 8-10 weeks, including live sessions, self-paced work, and mentoring sessions.  

Details on how to apply to the pilot cohort will be shared in the coming months. In the meantime, you can [subscribe to our newsletter](https://openlifescience.civicrm.org/civicrm/newsletter) to receive updates about this and other OLS activities. 

## Timeline

* June: **Webinar** - [Register here](https://us02web.zoom.us/meeting/register/ln9HJVbIR9-IAeWedXIG7g)  
* July: **Community consultation workshops** - [Learn more and register here]({% link _posts/2026-06-10-seeds-to-system-training-community-consultation-workshops.md %})
* August: **Cohort application period**  
* September - November: **Pilot cohort**

## Background

Over the past year, we have been exploring the conditions that make open projects and communities sustainable over time. Moving from a focus on grant writing to [grant readiness]({% link _posts/2026-04-20-from-hidden-curriculum-to-grant-readiness.md %}), and drawing on systems thinking to identify [leverage points]({% link _posts/2026-05-15-leverage-points.md %}) where targeted support can make a meaningful difference, we have been building an evidence-based understanding of what projects and communities need to sustain their work. We also had the opportunity to [pilot workshop activities]({% link _posts/2025-12-18-elife-workshop-on-grant-readiness.md %}) with the eLife Community Ambassadors, testing early ideas with a community audience. The Seeds to Systems pilot cohort is the next step in this work. 


