...
{% include _includes/announcement-blog-metadata.html cohort='ols-3' %}

We are thrilled to announce that [{{ p-participants | size }} members]({% link openseeds/ols-3/projects-participants.md %}#participants), who are the project leads of [{{ projects | size }} diverse projects]({% link openseeds/ols-3/projects-participants.md %}#projects), have joined the third cohort of the Open Life Science mentoring program - OLS-3!
...
We welcome our {{ p-mentors | size }} mentors, {{ p-mentors | join: ', ' }}, based in {{ m-countries | size }} countries ({{ m-countries | join: ', ' }}). {{ prev-part-count }} of them were participants and {{ prev-mentor-count }} mentors in the [previous cohort (OLS-2)]({% link openseeds/ols-2/index.md %}). They will be supported by [{{ experts | size }} experts](/openseeds/{{ cohort }}#experts).
...
We begin our program this week with a mentoring training call and mentor-mentee introductions. Check out the complete schedule and plans for OLS-3 here: [{{ site.url }}/{{ cohort }}](/openseeds/{{ cohort }}).
...
## Project details ([click here for full description]({% link openseeds/ols-3/projects-participants.md %}#projects))
