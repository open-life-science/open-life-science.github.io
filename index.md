...
{%- for cohort in cohorts -%}
  ...
  {%- endfor %}
| [{{ cohort_name | upcase }}](/openseeds/{{ cohort_name }}.html) | [{{ cohort_start }} - {{ cohort_end }}](/openseeds/{{ cohort_name }}/schedule) | {%- if cohort_participants > 0 -%}[{{ cohort_participants }} mentees](/openseeds/{{ cohort_name }}/projects-participants#participants) on {%- endif -%} {%- if cohort_projects > 0 -%}[{{ cohort_projects }} projects](/openseeds/{{ cohort_name }}/projects-participants#projects) {%- endif -%}| {%- if cohort_mentors > 0 -%}[{{ cohort_mentors }} mentors](/openseeds/{{ cohort_name }}#mentors) {%- endif -%}| {%- if experts > 0 -%}[{{ experts }} experts](/openseeds/{{ cohort_name }}#experts) {%- endif -%}| {%- if facilitators > 0 -%}[{{ facilitators }} facilitator(s)](/openseeds/{{ cohort_name }}#facilitators) {%- endif -%} |
{%- endfor -%}
